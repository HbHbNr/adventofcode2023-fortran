!> Solution for https://adventofcode.com/2023/day/12 part a
module day12a
...
contains
    ...
    integer(int64) function solve(filename)
        ...
        do row = 1, size(lines)
            ...
            allocate(damaged(split - 1), source=.false.)
            allocate(unknown(split - 1), source=.false.)
            do i = 1, split - 1
                if (line(i:i) == '#') then
                    damaged(i) = .true.
            ...
            call string_extract_integers(line(split+1:len_trim(line)), group_sizes)
